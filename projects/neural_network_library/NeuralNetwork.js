class NeuralNetwork {
    ...
    train(inputsArr, targetsArr) {
        ...
        gradient_ho.show()
        outputErrors.show()
        gradient_ho = Matrix.multiply(gradient_ho, outputErrors)
        let delta_weights_ho = Matrix.multiply(gradient_ho, Matrix.transpose(hiddenOutput))

        this.weights_ho = Matrix.add(this.weights_ho, Matrix.map(delta_weights_ho, elt => elt * this.learning_rate))
        ...
        // gradient_ih.show()
        // hiddenErrors.show()
        // gradient_ih = Matrix.multiply(gradient_ih, hiddenErrors)
        let delta_weights_ih = Matrix.multiply(gradient_ih, Matrix.transpose(inputs))


        this.weights_ih = Matrix.add(this.weights_ih, Matrix.map(delta_weights_ih, elt => elt * this.learning_rate))
        //adjust the bias for the hidden layer
        this.bias_h = Matrix.add(this.bias_h, gradient_ih)

        // this.weights_ih.show()
        // this.weights_ho.show()
    }
